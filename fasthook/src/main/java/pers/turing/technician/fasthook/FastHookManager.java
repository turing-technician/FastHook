...
public class FastHookManager {
    ...
    public static void doHook(String hookInfoClassName, ClassLoader hookInfoClassLoader, ClassLoader targetClassLoader, ClassLoader hookClassLoader, ClassLoader forwardClassLoader, boolean jitInline) {
        try {
            ...
            for(int i = 0; i < hookItems.length; i++) {
                ...
                Member forwardMethod = getMethod(hookItem[7],hookItem[8],hookItem[9],null,null,forwardClassLoader);
            ...
            case MODE_REWRITE:
                ...
                if(quickTrampolineList != null) {
					...
					if(Build.VERSION.SDK_INT < ANDROID_N) {
						boolean success = !comparePointer(getMethodEntryPoint(targetMethod),getQuickToInterpreterBridge());
						if(success) {
                    	...
                    	switch (jitState) {
                        	...
                        	case JIT_COMPILE:
                            	...
                            	boolean needCompile = comparePointer(getMethodEntryPoint(targetMethod),getQuickToInterpreterBridge());
	...
	private native static boolean comparePointer(long src, long dest);
    private native static boolean isCorrectSize(Member method);
    private native static boolean isNativeMethod(Member method);
    private native static void setNativeMethod(Member method);
    private native static boolean isNonPcRelated(Member method);
